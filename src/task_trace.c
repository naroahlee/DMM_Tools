...
int main(int argc, char** argv)
{
	...
	//param.budget_policy = QUANTUM_ENFORCEMENT;
	...
	sprintf(acFileName, "Task%d.trace", gs32RefID);
	dump_tracing_to_file(acFileName);
	//dump_tracing_to_file("Out.dat");
...
{
	int i;
	add_record_task(TASK_EVENT_RELEASE_JOB, gs32RefID);
	/* Do real-time calculation. */
	for(i = 0; i < gs32wcet_ms; i++)
	{
		workload_for_1ms();
	}
	add_record_task(TASK_EVENT_FINISH_JOB, gs32RefID);
	gs32count--;
